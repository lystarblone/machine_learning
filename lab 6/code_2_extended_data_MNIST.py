...
def move(array, shift, axis):  # сдвигает на один пиксель изображения
  moved = np.roll(array, shift, axis=axis)
  if axis == 1:
      if shift == 1:
          moved[:, 0] = 0
      else:
...
def read_mnist():
  ...
  for i in range(len(train_images)):
      u = move(train_images[i], -1, 0)
      d = move(train_images[i], 1, 0)
      r = move(train_images[i], 1, 1)
      l = move(train_images[i], -1, 1)
      train_images = np.concatenate((train_images, np.array([u, d, r, l])))
      train_labels = np.concatenate((train_labels, np.array([train_labels[i] for _ in range(4)])))

  x_train = train_images.reshape(300000, 784)
  mean = np.mean(x_train)  # среднее значение элементов массива
  stddev = np.std(x_train)  # стандартное отклонение
  x_train = (x_train - mean) / stddev
  x_test = test_images.reshape(10000, 784)
  x_test = (x_test - mean) / stddev

  y_train = np.zeros((300000, 10))
...
for i in range(36):
   x = x_test[i]
   forward_pass(np.concatenate((np.array([1.0]), x)))

   prediction = output_layer_y.argmax()
   ...
   ax.axis("off")
   ax.set_title(f"{true_label} ({sign})", fontsize=12)
